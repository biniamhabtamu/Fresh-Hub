...
import { CheckCircle2, Clock, AlertCircle, Phone, Copy, Banknote, Smartphone } from 'lucide-react';
import { db } from '../../firebase/config';
import { doc, getDoc, onSnapshot, updateDoc, setDoc } from 'firebase/firestore'; // Import setDoc and updateDoc
...
export default function PremiumPage() {
  ...
  const [premiumStatus, setPremiumStatus] = useState<'pending' | 'approved' | 'rejected'>('pending'); // New state for premium status
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userRef, (docSnap) => {
      ...
      if (data) {
        // Update states based on the premiumStatus from Firestore
        setPremiumStatus(data.premiumStatus || 'pending');
        setIsApproved(data.isPremium || false);
        // If the premiumStatus is not 'pending', it means admin has checked it
        setAdminChecked(data.premiumStatus !== 'pending'); 
        // If there's a premium request that was submitted, set submitted to true
        if (data.transactionId && data.premiumStatus === 'pending') {
          setSubmitted(true);
        } else if (data.isPremium || data.premiumStatus === 'rejected') {
          setSubmitted(true); // Keep submitted true if approved/rejected to show final state
        }
      }
    }, (error) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update the user's document with the transaction ID and pending status
      await updateDoc(doc(db, 'users', currentUser.uid), {
        transactionId: transactionId.trim(),
        premiumStatus: 'pending',
        isPremium: false, // Ensure isPremium is false when pending
        paymentMethod: activePaymentMethod, // Save the payment method used
        submittedAt: new Date(), // Timestamp for admin
      });

      setSubmitted(true);
      setAdminChecked(false); // Reset adminChecked to false as a new request is pending
      toast.success('Transaction ID submitted! Waiting for admin approval.');
    } catch (error) {
          ...
          <div className="p-6 sm:p-8">
            ...
            {premiumStatus === 'approved' ? (
              ...
              <div className="text-center">
                ...
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mt-4">Verification Failed</h2>
                <p className="text-gray-600 mt-2">
                  We couldn't verify your transaction. Please contact admin for assistance or try again.
                </p>
                ...
                <div className="mt-4 sm:mt-6">
                  <button
                    onClick={() => {
                      ...
                      setTransactionId(''); // Clear transaction ID for new attempt
                      setAdminChecked(false);
                      setPremiumStatus('pending'); // Reset status for new attempt
                    }}
            ...
            ) : premiumStatus === 'pending' && submitted ? (
              <div className="text-center">
                <div className="inline-flex items-center justify-center p-4 bg-purple-100 rounded-full">
                  <Clock className="h-12 w-12 text-purple-600" />
                </div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mt-4">Processing Your Request</h2>
                <p className="text-gray-600 mt-2">
                  Your transaction ID <span className="font-mono font-bold text-purple-700">{transactionId}</span> has been received. Our team is verifying your premium request. This usually takes up to 7 hours.
                </p>
